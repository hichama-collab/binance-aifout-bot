...
def score_symbol(sym: str, quote: str, interval: str, mom_window: int, klines_limit: int,
                 ...
                 min_range_pct: float, min_abs_mom_pct: float, min_vol: float):
    ...
    # Hard filters for "zigzag" style.
    # rangePct is in percent units; vol is fractional (e.g. 0.002 = 0.2% std per candle).
    range_pct = rng * 100.0
    abs_mom_pct = abs_mom * 100.0
    if min_range_pct > 0 and range_pct < min_range_pct:
        return None
    if min_abs_mom_pct > 0 and abs_mom_pct < min_abs_mom_pct:
        return None
...
def main():
    ...
    # Zigzag hard filters (defaults tuned to keep only symbols that actually move).
    ap.add_argument("--minRangePct", type=float, default=0.15,
                    help="Require short-term range >= this percent (e.g. 0.15 = 0.15%%)")
    ap.add_argument("--minAbsMomPct", type=float, default=0.03,
                    help="Require abs momentum >= this percent (e.g. 0.03 = 0.03%%)")
    ap.add_argument("--minVol", type=float, default=0.0015,
                    help="Require realized volatility >= this (fraction; 0.0015 ~= 0.15%% std per candle)")
    ...
    with ThreadPoolExecutor(max_workers=max(4, args.threads)) as ex:
        ...
        for s in syms:
            futs.append(ex.submit(
                ...
                args.minRangePct, args.minAbsMomPct, args.minVol
            ))
